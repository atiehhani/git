...
echo "Ì†ΩÌ¥ç Finding all 'created' pipelines (including paginated results)..."
...
if [ -z "$pipeline_ids" ] || [ "$total_count" -eq 0 ]; then
  echo "‚ùå No 'created' pipelines found"
  exit 0
fi

echo ""
echo "Ì†ΩÌ≥ä Found $total_count pipelines with status 'created'"
echo ""
...
echo ""
read -p "‚ö†Ô∏è  Delete ALL $total_count pipelines? (y/N): " confirm
